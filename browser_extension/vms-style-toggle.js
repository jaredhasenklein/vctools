...
(function() {
  ...
  let currentMode = 'original'; // Default to original style (changed from 'improved')
  let styleTag = null;
  let toggleBtn = null;
  ...
  function init() {
    ...
    // Check if we're on a VMS page - ONLY apply to my.firstinspires.org/VMS URLs
    if (!window.location.href.match(/^https?:\/\/my\.firstinspires\.org\/VMS/i)) {
      console.log("VMS Style Toggle: Not a VMS page, not initializing");
      return;
    }
  ...
  function addFloatingToggle() {
    ...
    toggleBtn.textContent = 'Original Style'; // Changed from 'Improved Style'
